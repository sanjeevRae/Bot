...
/**
 * Bookings — appointments the chatbot created with customers.
 *
 * Lifted verbatim from the former /bookings page so it can render inside the
 * Engagement tabs (and inside the "All" overview) without duplicating the feature.
 * `preview` caps the list and renders a "View all" link back to the Bookings tab.
 */
export default function EngagementBookings({ preview, onViewAll }) {
  ...
  const shown = preview ? bookings.slice(0, preview) : bookings;
  ...
  return (
    <section>
      ...
      {preview && bookings.length > preview && (
